...
export async function loadWork(env, scheduledTime, {read = readBytes, now = Date.now} = {}) {
  const worker = Number(env.WORKER_ID), repo = env.REPOSITORY;
  assert(worker === 0 || worker === 1, "Invalid WORKER_ID");
  assert(/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/.test(repo), "Invalid repository");
  assert(Number.isFinite(scheduledTime) && scheduledTime <= now() + 300000 && now() - scheduledTime < 120000, "Stale scheduled invocation");
  ...
  return {targets: work.targets, controls: plan.controls, worker_id: worker, bucket, round: roundOf(scheduledTime),
...
export async function connectOnce(endpoint, connect, {timeout = 3000, now = Date.now} = {}) {
  ...
  try {
    ...
    return {...base, status: "reachable", error: null, connect_ms: Math.max(0, Math.min(3000, Math.round(now() - started)))};
  } catch (error) {
    const reason = error?.message === "probe_timeout" ? "timeout" : classify(error);
    return {...base, status: reason === "platform" ? "unknown" : "unreachable", error: reason, connect_ms: null};
  } finally {
    clearTimeout(timer);
    if (socket) {
      // close() aborts the socket; a rejected or stuck completion must not hold
      // the entire batch open. Both socket promises already have rejection handlers.
      let closingTimer;
      try {
        await Promise.race([Promise.resolve(socket.close()),
          new Promise(resolve => { closingTimer = setTimeout(resolve, 100); })]);
      } catch { /* A failed close is already an unknown/failed connection. */ }
...
export async function runBatch(work, connect, {now = Date.now, attempt = connectOnce, budget = 45000, maxTargets = 40} = {}) {
  ...
  await Promise.all(Array.from({length: 4}, async () => {
    while (next < queue.length && now() - started <= budget - 3000) {
      const endpoint = queue[next++];
      const result = await attempt(endpoint, connect, {now});
...
export async function scheduled(controller, env, connect, dependencies = {}) {
  ...
  return {status: "stored", batch_id: batch.batch_id, bucket: batch.bucket, completed: batch.completed,
    deferred: batch.deferred, guarded: batch.guarded};
}
...
export async function serve(request, env, now = Date.now) {
  ...
  if (env.WORKER_ID !== "0") return response({error: "not_found"}, 404);
